...
#'
#' Convenience wrapper that first constructs an `fmridesign::event_model` from a
...
#' @param formula Model formula passed to [fmridesign::event_model()]. The default
...
#' @param ... Additional arguments passed to [fmridesign::event_model()].
#'
...
hrfals_design <- function(events, TR, basis, fmri_data_obj,
  ...
  if (inherits(fmri_data_obj, "fmri_dataset")) {
    ...
    sframe <- fmridesign::sampling_frame(nrow(fmri_data_obj), TR = TR)
  } else {
    stop("'fmri_data_obj' must be an 'fmri_dataset' or matrix")
  }

  emod <- fmridesign::event_model(formula = formula,
                               data = events,
                               block = block,
