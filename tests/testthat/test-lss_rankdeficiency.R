...
test_that("fastlss_shared handles rank-deficient A", {
  n <- 30; m <- 4; Tt <- 3; v <- 2
  ...
  p_vec <- rnorm(n)
  lambda <- 0.1
  res_fast <- fastlss_shared(Y, A, C, p_vec, lambda_ridge = lambda)
...
test_that("fastlss_voxel handles rank-deficient A", {
  ...
  p_vec <- rnorm(n)
  lambda <- 0.1
  res_fast <- fastlss_voxel(Y, A, X_onset_list, H_allvoxels, p_vec,
                         lambda_ridge = lambda)
